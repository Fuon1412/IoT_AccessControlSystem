...
import { useAccessFeed, type ConnState } from '../lib/signalr'
...
function Dashboard() {
  ...
  const [feed, setFeed] = useState<AccessLogDto[]>([])
  const seeded = useRef(false)

  useEffect(() => {
    if (!seeded.current && logs.data) {
      setFeed(logs.data.slice(0, FEED_MAX))
      seeded.current = true
    }
  }, [logs.data])

  const onLog = useCallback((log: AccessLogDto) => {
    setFeed((prev) => [log, ...prev].slice(0, FEED_MAX))
  }, [])
  ...
  const granted = feed.filter((l) => l.accessGranted).length
  const denied = feed.filter((l) => !l.accessGranted).length
  ...
  return (
    <div className="space-y-6">
      ...
      <Panel
        ...
        subtitle="Real-time RFID scan events"
      ...
      >
        ...
        <div className="space-y-1.5">
          {feed.map((log, i) => (
            <FeedRow key={`${log.id}-${log.requestId}`} log={log} fresh={i === 0 && seeded.current} />
          ))}
        </div>
      </Panel>
    </div>
