...
public class RfidCardService : IRfidCardService
{
    ...
    {
        if (await _db.RfidCards.AnyAsync(c => c.Uid == request.Uid))
            return null; // UID already registered
    ...
    public async Task<CardValidationResult> ValidateAsync(string uid)
    {
        var card = await _db.RfidCards
            .Include(c => c.User)
            .FirstOrDefaultAsync(c => c.Uid == uid && c.IsActive && c.User.IsActive);

        if (card is null)
            return new CardValidationResult(false, null, null, null);

        return new CardValidationResult(true, card.UserId, card.User.Username, card.User.Role);
    }

    private static RfidCardDto ToDto(RfidCard c) =>
        new(c.Id, c.Uid, c.IsActive, c.RegisteredAt, c.UserId, c.User.Username);
}
